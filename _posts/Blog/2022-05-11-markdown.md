...
1. \` 키 를 `세 번 입력` 후 `코드 언어 소문자 입력`  
코드입력  
\` 키 를 `세 번 입력` 하여 코드 블록 닫기 

2. `Tab` 키를 `두 번` 입력하여 코드 블럭을 생성할 수 있습니다.  
...
[15. 버튼만들기](#15-buttons-버튼-만들기){: .btn .btn--info .btn--large}
```

[맨 위로 이동](#){: .btn .btn--primary}
[맨 아래로 이동](#15-buttons-버튼-만들기){: .btn .btn--warning}
